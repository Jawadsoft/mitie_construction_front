...
let CashflowService = class CashflowService {
    ...
    ds;
    accounting;
    constructor(repo, ds, accounting) {
        this.repo = repo;
        this.ds = ds;
        this.accounting = accounting;
    }
    cashBankAccountFilter(alias = 'a') {
        return `(
      ${alias}.code = '1000'
    ...
    async findAll(filters) {
        const params = [];
        const journalWhere = [`je.status = 'Posted'`, this.cashBankAccountFilter('a')];
        const manualWhere = [`(t.related_entity_type IS NULL OR t.related_entity_type = '')`];
    ...
    async clearCashJournal(tx, manager) {
    ...
    async create(dto) {
        ...
        if (!dto.transaction_date) {
            throw new common_1.BadRequestException('transaction_date is required');
        }
        if (dto.type !== 'IN' && dto.type !== 'OUT') {
            throw new common_1.BadRequestException('type must be IN or OUT');
        }
        const amt = amount.toFixed(2);
        return this.ds.transaction(async (manager) => {
            const repo = manager.getRepository(cash_transaction_entity_1.CashTransaction);
            const tx = await repo.save(repo.create({
                ...dto,
                amount: amt,
    ...
    async update(id, dto) {
        return this.ds.transaction(async (manager) => {
            ...
            if (!existing)
                throw new common_1.BadRequestException('Cash transaction not found');
    ...
    async getDashboardStats() {
        ...
        const [[activeProjects], [totalBudget], [totalExpenses], [totalLabour], [totalRevenue], [collectedRevenue], [pendingReceivables], [supplierPayables], [totalUnits], [soldUnits], stageCompletion, [stockValue], [totalMaterials]] = await Promise.all([
