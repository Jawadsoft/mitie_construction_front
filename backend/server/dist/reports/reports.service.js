...
let ReportsService = class ReportsService {
    ...
    `);
        const sold_units = sold_unit_rows.map((r) => {
        ...
        return {
            ...
            sold_units_summary: {
                ...sold_units_summary,
                margin_pct: sold_units_summary.sale_price > 0
    ...
    async getCashflowReport(period = 'monthly', from, to) {
        ...
        if (from)
            dateWhere.push(`je.entry_date >= '${from}'`);
        if (to)
            dateWhere.push(`je.entry_date <= '${to}'`);
    ...
    async getPartnersEquity(as_of) {
        const banks = await this.q(`
