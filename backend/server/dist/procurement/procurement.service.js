...
let ProcurementService = class ProcurementService {
    ...
    receiptRepo;
    inventoryService;
    constructor(poRepo, itemRepo, receiptRepo, inventoryService) {
        this.poRepo = poRepo;
    ...
    async createReceipt(purchase_order_id, dto) {
        ...
        if (!po)
            throw new common_1.NotFoundException('Purchase order not found');
        const receipt = await this.receiptRepo.save(this.receiptRepo.create({
            purchase_order_id,
        ...
        for (const line of dto.items || []) {
            if (!line.material_id || !Number(line.quantity))
                continue;
        ...
        for (const poItem of poItems) {
            const match = (dto.items || []).find((i) => poItem.material_id === i.material_id ||
...
exports.ProcurementService = ProcurementService = __decorate([
    ...
    __param(3, (0, common_1.Inject)((0, common_1.forwardRef)(() => inventory_service_1.InventoryService))),
